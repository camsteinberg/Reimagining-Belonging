...
function formatTop(list: FlowData[], dir: "out" | "in") {
...
function buildFlowIndex(flows: FlowData[]) {
  const out = new Map<string, FlowData[]>();
  const inn = new Map<string, FlowData[]>();
...
export default function MigrationMap({ slideNum }: MigrationMapProps) {
    ...
    const initMap = async () => {
      try {
        ...
        map.on("load", () => {
          ...
          Promise.all([
          ...
          ]).then(([flows, statesGeo]: [FlowData[], GeoJSON.FeatureCollection]) => {
            ...
            let currentHoveredState: string | null = null;
            ...
            const getAbsoluteRatio = (population: number) =>
              (population - minPopulation) / (maxPopulation - minPopulation);

            const createColoredLayer = (data: FlowData[], id: string, isOutbound: boolean, isOutboundFlow: boolean) =>
              new ArcLayer({
                id,
                data,
                pickable: false,
                getSourcePosition: (d: FlowData) => [d.from_lon, d.from_lat],
                getTargetPosition: (d: FlowData) => [d.to_lon, d.to_lat],
                getSourceColor: (d: FlowData) => {
                ...
                getTargetColor: (d: FlowData) => {
                  const ratio = getAbsoluteRatio(d.population);
                  const alpha = Math.floor(isOutboundFlow ? 10 + ratio * 80 : 180 + ratio * 75);
                  return isOutbound ? [255, 100, 80, alpha] : [80, 150, 255, alpha];
                },
                getWidth: (d: FlowData) => 0.15 + getAbsoluteRatio(d.population) * 3,
                parameters: { depthTest: false },
              });

            function updateArcs(abbr: string | null) {
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              const newLayers: any[] = [
                new GeoJsonLayer({
                  id: "states-fill",
                  ...
                  // @ts-expect-error -- deck.gl event typing
                  onHover: ({ object, x, y }: { object: GeoJSON.Feature | null; x: number; y: number }) => {
                    if (!object) {
              ...
              if (abbr) {
                const outboundFlows = flows.filter((d: FlowData) => d.from_abbr === abbr);
                const inboundFlows = flows.filter((d: FlowData) => d.to_abbr === abbr);
  ...
  if (!mounted) return <div className="h-screen bg-cream" />;
