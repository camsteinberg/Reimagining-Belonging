...
/* Pick a spotlight participant for the quote section */
const SPOTLIGHT_INDEX = 0;
...
function FeaturedStoryCard({ participant, index }: { participant: Participant; index: number }) {
  const svgUrl = `/svg/${participant.svgIndex}inline.svg`;
      ...
      <div className="p-8 md:p-14 flex flex-col md:flex-row gap-8 md:gap-12">
        ...
        <div className="flex flex-col flex-1">
          ...
          <blockquote className="font-serif text-lg md:text-xl text-charcoal/80 italic leading-[1.6] mb-6">
            &ldquo;{participant.belongingQuote}&rdquo;
          </blockquote>
...
function StoryCard({ participant, index }: { participant: Participant; index: number }) {
  const svgUrl = `/svg/${participant.svgIndex}inline.svg`;
  const tags = THEME_TAGS[participant.slug] || [];
