...
import phoneImg1 from "@/assets/images/section01-scroll-1.webp";
import phoneImg2 from "@/assets/images/section01-scroll-2.webp";

gsap.registerPlugin(ScrollTrigger);

const phoneBgUrls = [phoneImg1.src, phoneImg2.src];

export default function PhoneOverlay() {
  const overlayRef = useRef<HTMLDivElement>(null);
  const phoneRef = useRef<HTMLDivElement>(null);
  const gradientRef = useRef<SVGCircleElement>(null);
  const lastPhoneBgRef = useRef("");
  const phoneFadeTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    const overlayRatios = new Map<Element, number>(overlaySlides.map((slide) => [slide, 0]));
    const darkSet = new Set<Element>(darkCircleSlides);
    const thresholds = [0, 0.2, 0.4, 0.6, 0.8, 1];
    let slide12Ratio = 0;
    let isSlide1Visible = false;

    // Slide 1 observer
    let s1Observer: IntersectionObserver | undefined;
    if (slide1) {
    ...
    let activeOverlaySlide: Element | null = null;
    ...
    const overlayObserver = new IntersectionObserver(
      (entries) => {
        ...
        if (phone) {
          ...
          if (activeSlide) {
            const phoneIndex = phoneSlides.indexOf(activeSlide as HTMLElement);
            if (phoneIndex >= 0) {
    ...
    const textTriggers = textSlides
      .map((slide) => {
        const topTrigger = slide.querySelector(topTextSelector) as HTMLElement | null;
        const bottomTrigger = slide.querySelector(bottomTextSelector) as HTMLElement | null;
    ...
    const slide16 = document.querySelector(".slide16") as HTMLElement | null;
    const slide16Text = document.querySelector(".slide16TextContainer") as HTMLElement | null;
    ...
    const fadePairs = syncedFades
      .map((set) => {
        ...
        const targets = set.targets.map((sel) => document.querySelector(sel)).filter(Boolean) as Element[];
        return trigger && targets.length ? { trigger, targets } : null;
      })
      .filter(Boolean) as Array<{ trigger: Element; targets: Element[] }>;

    const fadeObservers: IntersectionObserver[] = [];
