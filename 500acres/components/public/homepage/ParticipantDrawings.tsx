...
export default function ParticipantDrawings() {
  ...
  useEffect(() => {
    ...
    const slide19Drawings = slide19.querySelectorAll(".slide19Drawing") as NodeListOf<HTMLElement>;
    const slide19Quotes = slide19.querySelectorAll(".slide19QuoteContainer img") as NodeListOf<HTMLElement>;

    if (!slide19Drawings.length || !slide19Quotes.length) return;

    const centerDrawingInSlide = (drawingEl: HTMLElement) => {
    ...
    const isWithin = (rect: DOMRect, x: number, y: number) =>
      x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom;

    let activeDrawing: HTMLElement | null = null;
    let activeQuote: HTMLElement | null = null;
    ...
    const onDocumentTouch = (e: TouchEvent) => {
      ...
      const tappedDrawing = (e.target as HTMLElement).closest(".slide19Drawing");
    ...
    return () => {
      ...
      document.removeEventListener("mousemove", onPointerMove);
      document.removeEventListener("touchstart", onDocumentTouch as EventListener);
    };
  }, []);
    ...
    <section ref={slideRef} className="slide slide19" data-bg="">
      ...
      <div className="slide19Media">
        <div className="slide19Drawing" role="button" tabIndex={0} aria-label="Sherry's belonging drawing — hover or tap to explore">
          ...
          <img className="drawing1" src={drawing1.src} alt="Sherry's drawing about belonging" />
        </div>
        <div className="slide19Drawing" role="button" tabIndex={0} aria-label="Michaella's belonging drawing — hover or tap to explore">
          ...
          <img className="drawing2" src={drawing2.src} alt="Michaella's drawing about belonging" />
        </div>
        <div className="slide19Drawing" role="button" tabIndex={0} aria-label="Oliver's belonging drawing — hover or tap to explore">
          ...
          <img className="drawing3" src={drawing3.src} alt="Oliver's drawing about belonging" />
        </div>
        <div className="slide19Drawing" role="button" tabIndex={0} aria-label="FY's belonging drawing — hover or tap to explore">
          ...
          <img className="drawing4" src={drawing4.src} alt="FY's drawing about belonging" />
        </div>
      </div>

      <div className="slide19QuoteContainer" aria-hidden="true">
        <img className="slide19Quote1" src={quote1.src} alt="" />
        <img className="slide19Quote2" src={quote2.src} alt="" />
        <img className="slide19Quote3" src={quote3.src} alt="" />
        <img className="slide19Quote4" src={quote4.src} alt="" />
      </div>
    </section>
  );
}
