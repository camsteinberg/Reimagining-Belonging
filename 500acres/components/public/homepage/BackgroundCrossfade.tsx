...
export default function BackgroundCrossfade() {
  ...
  useEffect(() => {
    ...
    const slides = Array.from(document.querySelectorAll(".slide[data-bg]")) as HTMLElement[];
    if (slides.length === 0) return;

    const applyFadeDuration = (ms: number) => {
    ...
    const setBgInstant = (url: string | null | undefined) => {
      const active = activeRef.current;
      const inactive = inactiveRef.current;
      if (!url || !active) return;
      active.style.backgroundImage = `url("${url}")`;
      (active as HTMLElement & { dataset: DOMStringMap }).dataset.bgUrl = url;
    ...
    const changeBg = (url: string | null | undefined, fadeMs?: number) => {
      const active = activeRef.current;
      const inactive = inactiveRef.current;
      if (!url || !active) return;
      if ((active as HTMLElement & { dataset: DOMStringMap }).dataset.bgUrl === url) return;
      applyFadeDuration(fadeMs ?? DEFAULT_FADE_MS);
      if (!inactive || inactive === active) {
        setBgInstant(url);
        return;
      }
      inactive.style.backgroundImage = `url("${url}")`;
      (inactive as HTMLElement & { dataset: DOMStringMap }).dataset.bgUrl = url;
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (!visible) return;
        const url = (visible.target as HTMLElement).dataset.bg;
        const fadeMs = parseInt((visible.target as HTMLElement).dataset.fade || "", 10);
        changeBg(url, fadeMs);
      },
      { threshold: [0.5] }
    );
